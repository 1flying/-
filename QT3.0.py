...
class CameraDetectionThread(QThread):
    ...
    def __init__(self, camera_index, model_path, conf_threshold=0.5):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            while self.is_running:
                ...
                # ========== 3. 更新识别信息 ==========
                info_text = f"实时检测 - 帧数: {self.frame_count}\n"
                info_text += f"识别耗时: {detect_time}s\n"
                info_text += f"置信度阈值: {self.conf_threshold}\n"
                info_text += f"识别目标数: {len(results[0].boxes)}\n"
                for box in results[0].boxes:
...
class YOLOv8AircraftUI(QMainWindow):
    def __init__(self):
        ...
        self.camera_combo.addItem("默认摄像头 (0)", 0)
        self.camera_combo.addItem("备用摄像头 (1)", 1)
        self.camera_combo.addItem("备用摄像头 (2)", 2)
    ...
    def set_aircraft_theme(self):
        ...
        self.setStyleSheet("""
            ...
            QPushButton#camera_stop_btn {background-color: #ff4444;}
            QPushButton#camera_stop_btn:hover {background-color: #cc0000;}
            ...
            QLabel {color: #2c3e50;}
            QGroupBox {border: 1px solid #4a90e2; border-radius: 8px; margin-top: 8px; padding-top: 5px;}
    ...
    def toggle_camera(self):
        if self.camera_thread and self.camera_thread.isRunning():
            ...
            # 绑定信号
            self.camera_thread.original_frame_signal.connect(self.update_original_frame)
            self.camera_thread.detected_frame_signal.connect(self.update_detected_frame)
            self.camera_thread.info_signal.connect(self.update_info)
            ...
            self.stop_btn.setEnabled(True)
            self.result_display.setText(f"🔍 正在启动摄像头 {camera_index} 实时检测...")
    ...
    def on_conf_changed(self):
        ...
        if self.video_thread and self.video_thread.isRunning():
            ...
            self.result_display.append(f"\n🎯 置信度阈值更新为: {conf}")
        if self.camera_thread and self.camera_thread.isRunning():
            self.camera_thread.update_conf(conf)
            self.result_display.append(f"\n🎯 置信度阈值更新为: {conf}")
